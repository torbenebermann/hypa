...
std::map<std::string, PtxRegister> &Iss::S8Iss(const PtxInstruction &i, std::map<std::string, PtxRegister> &regs) {
  int8_t src0;
  /*int8_t src1;
  int8_t dst;*/
  ...
  switch (i.GetInstructionKeyword()) {
    ...
    // ...
    case kCvt: src0 = CastWrapperS8(regs[i.GetSrcRegister()[0]]);
      ...
      if (i.GetModifier()[i.GetModifier().size() - 2] == "u8") {
        ...
        Logger::Log("Iss::S8Iss | Conversion from s8 to " + i.GetModifier()[i.GetModifier().size() - 2]
                        + " not supported yet\n", "ERROR");
      }
      ...
      break;
    default: Logger::Log("Iss::S8Iss | Instruction not implemented: \"" + i.GetRawLine() + "\"\n", "ERROR");
  }
